...
(function($) {
	...
	$(this).everyTime(options.pollingInterval, function() {
		...
		if($.media.convertTimestamp($('input.clip_in').val()) - 1> t) {
			obj.pause();		
			obj.seek($.media.convertTimestamp($('input.clip_in').val()));
		
		}
		if($.media.convertTimestamp($('input.clip_out').val()) + 1< t) {
			obj.pause();		
			obj.seek($.media.convertTimestamp($('input.clip_out').val()));
		}
	});
...
//		$('input.clip_out', options.tools).change(function() { video.duration = $.media.convertTimestamp($(this).val()) - video.start;});	
		...
		$('input.clip_in', options.tools).add('input.clip_out', options.tools).change(function() { $('.scrubbar').trigger('slider:range', {begin: $.media.convertTimestamp($(this).parent().find('.clip_in').val()), end: $.media.convertTimestamp($(this).parent().find('.clip_out').val())}); });
	},
