...
//		$(this).append('<div class="scrubbar"></div>');
		...
		options.source.clip_in.change(function() { 
			...
			$('.scrubbar .clip_in span').html($('input.clip_in', that).val().replace(/^00:/, ""));
		})
		$('.scrubbar .ui-slider-handle:nth-child(1)', this).addClass('clip_in').append($('<span>0:00</span>'));
		$('.scrubbar .ui-slider-handle:nth-child(2)', this).addClass('playhead');
		$('.scrubbar .ui-slider-handle:nth-child(3)', this).addClass('clip_out').append($('<span>0:00</span>'));
			$('.scrubbar .clip_in span').html($('input.clip_in', that).val().replace(/^00:/, ""));
			$('.scrubbar .clip_out span').html($('input.clip_out', that).val().replace(/^00:/, ""));
		$(this).append('<div class="timestamp"><span class="current-timestamp">00:00:00</span><span class="duration-timestamp"></span></div>');
		$('.duration-timestamp', this).html(options.source.duration.val());
		options.source.clip_out.change(function() { $('.scrubbar', that).slider('values', 2, $.media.convertTimestamp($(this).val()));  $('.scrubbar .clip_out span').html($('input.clip_out', that).val().replace(/^00:/, "")); });
		/*
		if(options.event) {
        ...
        if(options.poll != false) {
			...
			var t = options.poll();
                    $(this).slider('values', 1, t);
		    $('.current-timestamp', that).html($.media.convertTime(t).substring(0,8));
            });
        }
